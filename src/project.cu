...
int main(int argc, char *argv[]){
  ...
  while(!glfwWindowShouldClose(window)) {
    gettimeofday(&start, NULL);
      ...
      update_u<<<Nblocks_tot, Nthreads_tot>>>(u_gpu, fx_gpu, 0, 0, nx);

      update_u<<<Nblocks_tot, Nthreads_tot>>>(u_gpu, fx_gpu, 0, 1, nx);

      update_u<<<Nblocks_tot, Nthreads_tot>>>(u_gpu, fy_gpu, 1, 0, nx);

      update_u<<<Nblocks_tot, Nthreads_tot>>>(u_gpu, fy_gpu, 1, 1, nx);
